...
import org.nakii.mmorpg.quest.Point;
...
import org.nakii.mmorpg.quest.api.common.component.VariableComponent;
...
import org.nakii.mmorpg.quest.logger.BetonQuestLogRecord;
import org.nakii.mmorpg.quest.logger.PlayerLogWatcher;
import org.nakii.mmorpg.quest.logger.format.ChatFormatter;
import org.nakii.mmorpg.quest.logger.handler.history.LogPublishingController;
...
import org.nakii.mmorpg.quest.quest.event.give.GiveEvent;
import org.nakii.mmorpg.quest.quest.objective.variable.VariableObjective;
import org.nakii.mmorpg.quest.util.Utils;
import org.nakii.mmorpg.quest.web.downloader.DownloadFailedException;
import org.nakii.mmorpg.quest.web.downloader.Downloader;
...
public class QuestCommand implements CommandExecutor, SimpleTabCompleter {
     ...
     */
    private final QuestModule instance;
    ...
    public QuestCommand(final BetonQuestLoggerFactory loggerFactory, final BetonQuestLogger log,
                        ...
                        final LogPublishingController debuggingController, final QuestModule plugin,
            ...
            if ("betonquest".equalsIgnoreCase(cmd.getName())) {
                ...
                switch (args[0].toLowerCase(Locale.ROOT)) {
                    ...
                    case "backup":
                        ...
                        Utils.backup(configAccessorFactory, MMORPGCore.getInstance());
                        break;
                    case "debug":
    ...
    private void displayVersionInfo(final CommandSender sender, final String commandAlias) throws QuestException {
        ...
        final VariableComponent baseContent = new VariableComponent(pluginMessage.getMessage(null, "command_version_output.info",
                new VariableReplacement("version", Component.text(MMORPGCore.getInstance().getDescription().getVersion())),
    ...
    private void handleDownload(final CommandSender sender, final String... args) {
        ...
        final Downloader downloader = new Downloader(loggerFactory.create(Downloader.class, "Downloader"),
                MMORPGCore.getInstance().getDataFolder(), githubNamespace, ref, offsetPath, sourcePath, targetPath, recursive, overwrite);
        ...
        Bukkit.getScheduler().runTaskAsynchronously(MMORPGCore.getInstance(), () -> {
            try {
                ...
                if (sender instanceof final Player player) {
                    final BetonQuestLogRecord record = new BetonQuestLogRecord(Level.FINE, null, MMORPGCore.getInstance());
                    record.setThrown(e);
    ...
    private void sendMessageSync(final CommandSender sender, final String messageName, final VariableReplacement... variables) {
        Bukkit.getScheduler().runTask(MMORPGCore.getInstance(), () -> sendMessage(sender, messageName, variables));
    }
