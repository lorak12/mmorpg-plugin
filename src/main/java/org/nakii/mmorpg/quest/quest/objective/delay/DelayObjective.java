...
public class DelayObjective extends Objective {
    ...
    public DelayObjective(final Instruction instruction, final Variable<Number> interval,
        ...
        }.runTaskTimer(MMORPGCore.getInstance(), 0, interval.getValue(null).longValue());
    }
    ...
    private Component parseVariableLeft(final Profile profile) throws QuestException {
        final PluginMessage pluginMessage = MMORPGCore.getInstance().getQuestModule().getPluginMessage();
    ...
    private String parseVariableDate(final Profile profile) {
        return new SimpleDateFormat(MMORPGCore.getInstance().getQuestModule().getPluginConfig().getString("date_format", ""), Locale.ROOT)
