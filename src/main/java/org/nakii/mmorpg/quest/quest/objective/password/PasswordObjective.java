    ...
    private boolean chatInput(final boolean fromCommand, final Player player, final String message) throws QuestException {
        ...
        try {
            prefix = passwordPrefix == null
                    ? LegacyComponentSerializer.legacySection().serialize(MMORPGCore.getInstance().getQuestModule().getPluginMessage().getMessage(onlineProfile, "password"))
                    : passwordPrefix;
        ...
        if (checkConditions(onlineProfile)) {
            if (regex.matcher(password).matches()) {
                Bukkit.getScheduler().runTask(MMORPGCore.getInstance(), () -> completeObjective(onlineProfile));
                return !fromCommand || !prefix.isEmpty();
            } else {
                try {
                    for (final EventID event : failEvents.getValue(onlineProfile)) {
                        MMORPGCore.getInstance().getQuestModule().getQuestTypeApi().event(onlineProfile, event);
                    }
                } catch (final QuestException e) {
