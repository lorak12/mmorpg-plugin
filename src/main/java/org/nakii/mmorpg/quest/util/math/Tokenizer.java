package org.nakii.mmorpg.quest.util.math;
...
import org.nakii.mmorpg.quest.quest.variable.math.MathVariable;
import org.nakii.mmorpg.quest.util.math.tokens.*;
import org.nakii.mmorpg.quest.util.math.tokens.Number;
...
public class Tokenizer {
    ...
    private Token tokenize(@Nullable final Token val1, @Nullable final Operator operator, final String val2) throws QuestException {
        ...
        if (chr == '{') {
            ...
            try {
                nextInLine = new Variable(new org.nakii.mmorpg.quest.api.instruction.variable.Variable<>(variableProcessor, pack, "%" + variableName + "%", Argument.NUMBER));
            ...
            try {
                nextInLine = new Variable(new org.nakii.mmorpg.quest.api.instruction.variable.Variable<>(variableProcessor, pack, "%" + val2.substring(start, index--) + "%", Argument.NUMBER));
            } catch (final QuestException e) {
                throw new QuestException("invalid calculation (" + e.getMessage() + ")", e);
            }
        }
