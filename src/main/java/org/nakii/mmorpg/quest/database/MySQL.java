...
public class MySQL extends Database {
     ...
     */
    public MySQL(final BetonQuestLogger log, final MMORPGCore plugin, final String hostname, final String port, final String database, final String username, final String password) {
        super(log, plugin);
        this.log = log;
    ...
    private void migration4(final Connection connection) throws SQLException {
        try (Statement statement = connection.createStatement()) {
            ...
            try (ResultSet resultSet = connection.createStatement().executeQuery("SELECT * FROM " + prefix + "backpack");
                ...
                final SimpleQuestItemFactory itemFactory = new SimpleQuestItemFactory(MMORPGCore.getInstance().getQuestModule().getQuestPackageManager(),
