...
public class QuestItemHandler implements Listener {
    ...
    public void onRespawn(final PlayerRespawnEvent event) {
        if (config.getBoolean("item.quest.remove_after_respawn")) {
            ...
            // items after respawning, so the player doesn't lose his
    ...
    /**
     * Prevents general in-world interaction with QuestItems and the Journal.
     * <p>
    ...
    public void onInteractEvent(final PlayerInteractEvent event) {
        ...
        // Replace EnchantmentTarget.TOOL.includes(item.getType()) with a manual tool check
        if (!isTool(type) && Utils.isQuestItem(item)) {
            event.setCancelled(true);
        }
    }

    // Helper method to determine if a material is a tool
