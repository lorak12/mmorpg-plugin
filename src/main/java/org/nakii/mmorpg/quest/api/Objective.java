...
import org.nakii.mmorpg.quest.api.bukkit.event.PlayerObjectiveChangeEvent;
import org.nakii.mmorpg.quest.api.common.function.QuestRunnable;
import org.nakii.mmorpg.quest.api.common.function.QuestSupplier;
...
import org.nakii.mmorpg.quest.api.quest.objective.ObjectiveData;
import org.nakii.mmorpg.quest.api.quest.objective.ObjectiveDataFactory;
import org.nakii.mmorpg.quest.api.quest.objective.ObjectiveID;
import org.nakii.mmorpg.quest.api.quest.objective.ObjectiveState;
...
public abstract class Objective {
    ...
    public Objective(final Instruction instruction, final ObjectiveDataFactory templateFactory) throws QuestException {
        ...
        this.profileProvider = MMORPGCore.getInstance().getQuestModule().getProfileProvider();
    ...
    public final void completeObjective(final Profile profile) {
        ...
        final PlayerData playerData = MMORPGCore.getInstance().getQuestModule().getPlayerDataStorage().get(profile);
        ...
        try {
            for (final EventID event : events.getValue(profile)) {
                MMORPGCore.getInstance().getQuestModule().getQuestTypeApi().event(profile, event);
            }
    ...
    public final boolean checkConditions(final Profile profile) {
        ...
        try {
            return MMORPGCore.getInstance().getQuestModule().getQuestTypeApi().conditions(profile, conditions.getValue(profile));
        } catch (final QuestException e) {
    ...
    public final void newPlayer(final Profile profile) {
        ...
        MMORPGCore.getInstance().getQuestModule().getPlayerDataStorage().get(profile).addObjToDB(instruction.getID().getFull(), defaultInstruction);
    }
    ...
    private void runObjectiveChangeEvent(final Profile profile, final ObjectiveState previousState, final ObjectiveState newState) {
        final boolean isAsync = !MMORPGCore.getInstance().getServer().isPrimaryThread();
    ...
    public void close() {
        for (final Map.Entry<Profile, ObjectiveData> entry : dataMap.entrySet()) {
            ...
            stop(profile);
            MMORPGCore.getInstance().getQuestModule().getPlayerDataStorage().get(profile).addRawObjective(instruction.getID().getFull(),
