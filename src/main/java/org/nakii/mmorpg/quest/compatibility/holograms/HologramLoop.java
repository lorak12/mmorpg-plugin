...
import org.nakii.mmorpg.quest.api.instruction.argument.types.NumberParser;
...
import org.nakii.mmorpg.quest.compatibility.holograms.lines.*;
...
public abstract class HologramLoop extends SectionProcessor<HologramLoop.HologramID, HologramWrapper> {
    ...
    public void clear() {
        super.clear();
        defaultInterval = MMORPGCore.getInstance().getQuestModule().getPluginConfig().getInt("hologram.update_interval", 10 * 20);
    }
    ...
    protected HologramWrapper loadSection(final QuestPackage pack, final ConfigurationSection section) throws QuestException {
        ...
        final HologramWrapper hologramWrapper = new HologramWrapper(
                ...
                MMORPGCore.getInstance().getQuestModule().getQuestTypeApi(),
                MMORPGCore.getInstance().getQuestModule().getProfileProvider(),
                ...
                pack,
                maxRange);
        HologramRunner.addHologram(MMORPGCore.getInstance(), hologramWrapper);
    ...
    private ItemLine parseItemLine(final QuestPackage pack, final String line) throws QuestException {
        try {
            ...
            return new ItemLine(MMORPGCore.getInstance().getQuestModule().getFeatureApi().getItem(itemID, null).generate(stackSize));
