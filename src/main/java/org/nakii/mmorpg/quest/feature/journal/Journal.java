...
import org.nakii.mmorpg.quest.api.bukkit.event.PlayerJournalAddEvent;
import org.nakii.mmorpg.quest.api.bukkit.event.PlayerJournalDeleteEvent;
...
public class Journal {
     ...
     */
    private static final BetonQuestLogger LOG = MMORPGCore.getInstance().getQuestModule().getLoggerFactory().create(Journal.class);
    ...
    public static boolean isJournal(final OnlineProfile onlineProfile, @Nullable final ItemStack item) {
        ...
        try {
            ...
            final Component journalTitle = MMORPGCore.getInstance().getQuestModule().getPluginMessage().getMessage(onlineProfile, "journal_title");
    ...
    private static List<Component> getJournalLore(final Profile profile) throws QuestException {
        return ComponentLineWrapper.splitNewLine(MMORPGCore.getInstance().getQuestModule().getPluginMessage().getMessage(profile, "journal_lore"));
    }
    ...
    public void addPointer(final Pointer pointer) {
        ...
        new PlayerJournalAddEvent(profile, !MMORPGCore.getInstance().getServer().isPrimaryThread(), this, pointer).callEvent();
        pointers.add(pointer);
        final String date = questModule.isMySQLUsed()
                ? new SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.ROOT).format(new Date(pointer.timestamp()))
                : Long.toString(pointer.timestamp());
        questModule.getSaver().add(new Record(UpdateType.ADD_JOURNAL, profile.getProfileUUID().toString(),
    ...
    public void removePointer(final JournalEntryID pointerName) {
        for (final Pointer pointer : pointers) {
            if (pointer.pointer().equals(pointerName)) {
                final QuestModule questModule = MMORPGCore.getInstance().getQuestModule();
                new PlayerJournalDeleteEvent(profile, !MMORPGCore.getInstance().getServer().isPrimaryThread(), this, pointer).callEvent();
                final String date = questModule.isMySQLUsed()
                        ? new SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.ROOT).format(new Date(pointer.timestamp()))
                        : Long.toString(pointer.timestamp());
                questModule.getSaver().add(new Record(UpdateType.REMOVE_JOURNAL, profile.getProfileUUID().toString(),
                        pointer.pointer().getFull(), date));
