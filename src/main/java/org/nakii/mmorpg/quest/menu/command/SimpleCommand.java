...
public abstract class SimpleCommand extends Command implements PluginIdentifiableCommand {
    ...
    @Override
    public MMORPGCore getPlugin() {
        return MMORPGCore.getInstance();
    }
    ...
    protected Component getMessage(final CommandSender sender, final String message, final VariableReplacement... replacements) {
        final PluginMessage pluginMessage = getPlugin().getQuestModule().getPluginMessage();
        final OnlineProfile profile = sender instanceof final Player player ? getPlugin().getQuestModule().getProfileProvider().getProfile(player) : null;
        try {
