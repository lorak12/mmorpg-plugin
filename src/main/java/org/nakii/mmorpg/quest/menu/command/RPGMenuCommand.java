package org.nakii.mmorpg.quest.menu.command;
        ...
        switch (args[0].toLowerCase(Locale.ROOT)) {
            ...
            case "o":
                if (!args[1].contains(Identifier.SEPARATOR)) {
                    return new ArrayList<>(getPlugin().getQuestModule().getQuestPackageManager().getPackages().keySet());
                }
                final String pack = args[1].substring(0, args[1].indexOf(Identifier.SEPARATOR));
                final QuestPackage configPack = getPlugin().getQuestModule().getQuestPackageManager().getPackages().get(pack);
                if (configPack == null) {
                    return new ArrayList<>();
                }
    ...
    public boolean simpleCommand(final CommandSender sender, final String alias, final String[] args) {
        ...
        switch (args[0].toLowerCase(Locale.ROOT)) {
            ...
            case "open":
                ...
                if (args.length >= 2) {
                    try {
                        menu = new MenuID(getPlugin().getQuestModule().getQuestPackageManager(), null, args[1]);
        ...
        switch (args[0].toLowerCase(Locale.ROOT)) {
            ...
            case "open":
                ...
                try {
                    this.menu.openMenu(getPlugin().getQuestModule().getProfileProvider().getProfile(player), menu);
